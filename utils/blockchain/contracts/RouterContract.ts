...
export class ReadRouterContract {
	...
	getFullPubKey = async (tokenId: any): Promise<any> => {
		...
		try {
			pubKey = await this.contract.getPubkey(tokenId);
		} catch (e: any) {
			// console.error("Error getting pubkey from contract", e);
			throw new Error("Error getting pubkey from contract: " + e.message);
		}
	...
	isActionRegistered = async (ipfsId: string): Promise<boolean> => {
		console.warn(
			"[*** DEPRECATED ***] isActionRegistered, this will always return true, there's no need to use this function anymore."
		);
