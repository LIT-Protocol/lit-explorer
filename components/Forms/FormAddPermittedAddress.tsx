...
const FormAddPermittedAddress = ({
	...
	useEffect(() => {
		...
		(async () => {
			...
			console.log(
				"[FormAddPermittedAddress] output<_isRegistered>:",
				_isRegistered
			);
	...
	const onSubmit = async () => {
		...
		// -- execute
		setProgress({
			message: `Permitting PKP "${heyShorty(_pkpId)}" to ${heyShorty(
				_ownerAddress
			)}...`,
			progress: 50,
		});
		...
		try {
			tx = await pkpPermissionsContract.write.addPermittedAddress(
				_pkpId,
				_ownerAddress,
				routerContract
			);
	...
	if (!isActionRegistered)
		return <>Found ipfsId, but Action code is not registered.</>;
