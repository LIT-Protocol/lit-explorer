...
const FormAddPermittedAction = ({
	...
	useEffect(() => {
		...
		(async () => {
			...
			console.log(
				"[FormAddPermittedAction] output<_isRegistered>:",
				_isRegistered
			);
	...
	const onSubmit = async () => {
		...
		setProgress({
			message: `Checking if action is registered...`,
			progress: 10,
		});
		const _isRegistered = await routerContract.read.isActionRegistered(
			ipfsId
		...
		// -- execute
		setProgress({
			message: `Permitting action to ${_pkpId}...`,
			progress: 50,
		});
		...
		try {
			tx = await pkpPermissionsContract.write.addPermittedAction(
				_pkpId,
				_ipfsId,
				routerContract
			);
