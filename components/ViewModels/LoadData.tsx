  ...
  useEffect(() => {
    ...
    if (!props?.useData) {
      ...
      cacheFetch(
        ...
        cache,
        {
          cacheExpire: props?.cacheExpire ?? 60 * 1000,
        }
      );
      return;
    }
    ...
    return (
      <>
        {props?.i18n?.loadingId ? (
          <h2>
            <FormattedMessage id={props?.i18n?.loadingId} />
          </h2>
        ) : (
      ...
      {columns.length > 0 && rows.length > 0 ? (
        <>
          <div className="flex">
            ...
            {props.renderStatus ? (
              <div className="flex-content">{props.renderStatus}</div>
            ) : (
              ""
            )}
          </div>
        ...
        <div className="mt-12">
          <Alert severity="error">{renderi18nError()}</Alert>
        </div>
      )}
    </>
  );
