...
export default function PKPOptionsModal(props: PKPOptionsModalProps) {
	...
	const handleClick = async () => {
		...
		if (inputType !== SupportedSearchTypes.ETH_ADDRESS) {
			resetProgress(
				`Incorrect input type. Expecting 'ETH_ADDRESS' but received ${inputType}`
			);
			return;
		}
		...
		setProgress(70);
		const permittedAddressTx =
			await pkpPermissionsContract.write.addPermittedAddress(
				pkpId,
				user_address,
				routerContract
			);
				...
				<Box sx={MODAL_STYLE}>
					...
					<Typography id="modal-modal-title" component="h2">
						<div className="text-center text-tiny">
							TokenId:{converter.decToHex(pkpId)}
						</div>
					</Typography>
					<Typography id="modal-modal-title" component="h2">
						{loading ? (
