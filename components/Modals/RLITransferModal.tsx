...
export default function RLITransferModal(props: RLITransferModal) {
	...
	const handleClick = async () => {
		...
		console.log(
			`[handleClick]: sending RLI Token ID "${tokenId}" to "${recipientAddress}" from "${ownerAddress}"`
		);
		...
		try {
			...
			resetProgress();
			throwError(
				`Failed to trasnfer token "${tokenId}" from "${ownerAddress}" to "${recipientAddress}" `
			);
			return;
		}
		...
		const isTransferred = await tryUntil({
			onlyIf: async () => {
				let _newOwner = (
					await rliContract.read.ownerOf(tokenId)
				).toLowerCase();
				let _recipientAddress = (
					recipientAddress as string
				).toLowerCase();

				console.log(
					`${heyShorty(_newOwner)} === ${heyShorty(
						_recipientAddress
					)}: ${_newOwner === _recipientAddress}`
				);
				...
				<Box sx={MODAL_STYLE}>
					...
					<Typography id="modal-modal-title" component="h2">
						<div className="text-center text-tiny">
							RLI Token ID: {RLI_ID.row.tokenID}
						</div>
					</Typography>
					...
					<div className="flex">
						...
						/>
						<Button onClick={handleClick} className="btn-2 ml-auto">
							Send
						</Button>
					</div>
				</Box>
